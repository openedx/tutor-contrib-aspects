...
CREATE USER IF NOT EXISTS {{ OARS_CLICKHOUSE_VECTOR_USER }} IDENTIFIED WITH sha256_password BY '{{ OARS_CLICKHOUSE_VECTOR_PASSWORD }}';
...
ALTER USER {{ OARS_CLICKHOUSE_VECTOR_USER }} IDENTIFIED WITH sha256_password BY '{{ OARS_CLICKHOUSE_VECTOR_PASSWORD }}';
...
CREATE DATABASE IF NOT EXISTS {{ OARS_VECTOR_DATABASE }};

CREATE TABLE IF NOT EXISTS {{ OARS_VECTOR_DATABASE }}.{{ OARS_VECTOR_RAW_TRACKING_LOGS_TABLE }}
(
    `time` DateTime,
    `message` String
)
...
CREATE TABLE IF NOT EXISTS {{ OARS_VECTOR_DATABASE }}.{{ OARS_VECTOR_RAW_XAPI_TABLE }}
(
    event_id      UUID,
...
GRANT INSERT, SELECT ON {{ OARS_VECTOR_DATABASE }}.* TO '{{ OARS_CLICKHOUSE_VECTOR_USER }}';
GRANT SELECT ON {{ OARS_VECTOR_DATABASE }}.* TO '{{ OARS_CLICKHOUSE_REPORT_USER }}';
