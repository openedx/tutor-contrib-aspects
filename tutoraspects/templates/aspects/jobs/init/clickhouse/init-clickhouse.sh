...
-- Create our various roles for RBAC
CREATE ROLE IF NOT EXISTS openedx_instructor;
CREATE ROLE IF NOT EXISTS openedx_superuser;
CREATE ROLE IF NOT EXISTS openedx_administrator;
...
CREATE DATABASE IF NOT EXISTS {{ ASPECTS_PERMISSIONS_DATABASE }};

CREATE TABLE IF NOT EXISTS {{ ASPECTS_PERMISSIONS_DATABASE }}.{{ ASPECTS_PERMISSIONS_COURSE_MAPPING_TABLE }}
(
  clickhouse_username String NOT NULL,
  course_id String NOT NULL,
)
engine = MergeTree
PRIMARY KEY (clickhouse_username, course_id)
ORDER BY (clickhouse_username, course_id);

GRANT SELECT ON {{ ASPECTS_PERMISSIONS_DATABASE }}.{{ ASPECTS_PERMISSIONS_COURSE_MAPPING_TABLE }}
TO openedx_instructor, openedx_superuser, openedx_administrator;

CREATE ROW POLICY OR REPLACE openedx_policy_restricted ON
...
{{ ASPECTS_EVENT_SINK_DATABASE }}.{{ ASPECTS_EVENT_SINK_OVERVIEWS_TABLE }},
{{ ASPECTS_EVENT_SINK_DATABASE }}.{{ ASPECTS_EVENT_SINK_NODES_TABLE }},
{{ ASPECTS_EVENT_SINK_DATABASE }}.{{ ASPECTS_EVENT_SINK_RELATIONSHIPS_TABLE }},
{{ ASPECTS_PERMISSIONS_DATABASE }}.{{ ASPECTS_PERMISSIONS_COURSE_MAPPING_TABLE }}
USING 0 TO ALL EXCEPT {{ CLICKHOUSE_ADMIN_USER}}, openedx_administrator, openedx_superuser;
...
{{ ASPECTS_EVENT_SINK_DATABASE }}.{{ ASPECTS_EVENT_SINK_RELATIONSHIPS_TABLE }},
{{ ASPECTS_PERMISSIONS_DATABASE }}.{{ ASPECTS_PERMISSIONS_COURSE_MAPPING_TABLE }}
USING 1 TO {{ CLICKHOUSE_ADMIN_USER}}, openedx_administrator, openedx_superuser;
...
{{ ASPECTS_PERMISSIONS_DATABASE }}.{{ ASPECTS_PERMISSIONS_COURSE_MAPPING_TABLE }}
USING
    course_id in (select course_id from {{ ASPECTS_PERMISSIONS_DATABASE }}.{{ ASPECTS_PERMISSIONS_COURSE_MAPPING_TABLE }} where username=currentUser())
