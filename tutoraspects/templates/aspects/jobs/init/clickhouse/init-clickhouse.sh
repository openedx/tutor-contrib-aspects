...
-- Create databases
CREATE DATABASE IF NOT EXISTS {{ RALPH_DATABASE }} {{ ON_CLUSTER }};
...
-- Update user passwords to match configuration if they do exist
...
-- Grant permissions to the Ralph user
GRANT {{ ON_CLUSTER }} INSERT, SELECT ON {{ RALPH_DATABASE }}.* TO '{{ ASPECTS_CLICKHOUSE_LRS_USER }}';
GRANT {{ ON_CLUSTER }} SELECT ON {{ DBT_PROFILE_TARGET_DATABASE }}.* TO '{{ ASPECTS_CLICKHOUSE_LRS_USER }}';
GRANT {{ ON_CLUSTER }} SELECT ON {{ ASPECTS_EVENT_SINK_DATABASE }}.* TO '{{ ASPECTS_CLICKHOUSE_LRS_USER }}';

-- Grant permissions to the Vector user
...
GRANT {{ ON_CLUSTER }} SELECT ON {{ ASPECTS_EVENT_SINK_DATABASE }}.* TO '{{ ASPECTS_CLICKHOUSE_VECTOR_USER }}';
...
-- Grant permissions to the dbt / Superset user
    -- Source databases
GRANT {{ ON_CLUSTER }} SELECT, DROP TABLE, DROP VIEW ON {{ RALPH_DATABASE }}.* TO '{{ ASPECTS_CLICKHOUSE_REPORT_USER }}';
GRANT {{ ON_CLUSTER }} SELECT, DROP TABLE, DROP VIEW ON {{ ASPECTS_VECTOR_DATABASE }}.* TO '{{ ASPECTS_CLICKHOUSE_REPORT_USER }}';
