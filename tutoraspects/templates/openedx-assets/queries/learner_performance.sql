select
    ...
    count(distinct case when attempt > 1 then last_response.object_id end) as response_count,
...
join
    {{ ASPECTS_EVENT_SINK_DATABASE }}.dim_course_names blocks using (course_key)
left join
...
group by org, course_key, course_name, course_run, actor_id,course_grade, approving_state
