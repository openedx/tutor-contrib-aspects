with
    enrollments as ({% include 'openedx-assets/queries/fact_enrollments.sql' %}),
    enrollments_ranked as (
        select
            ...
            enrollment_status,
            rank() over (
                partition by date(emission_time), org, course_name, course_run, actor_id
    ...
    enrollment_windows as (
        select
            ...
            lagInFrame(emission_time, 1, now() + interval '1' day) over (
                partition by org, course_name, course_run, actor_id
                order by emission_time desc
            ) as window_end_at
