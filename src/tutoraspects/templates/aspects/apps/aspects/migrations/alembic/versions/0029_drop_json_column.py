...
upgraded_table_name = "{{ RALPH_DATABASE }}.old_{{ ASPECTS_RAW_XAPI_TABLE }}"


def upgrade():
    op.execute(
        """
        DROP VIEW IF EXISTS {{ RALPH_DATABASE }}.xapi_events_all_parsed_mv;
        """
    )
    op.execute(
        f"""
        ALTER TABLE  {{ RALPH_DATABASE }}.{{ ASPECTS_RAW_XAPI_TABLE }}
        {on_cluster}
